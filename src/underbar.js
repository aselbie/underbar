...
(function() {
  ...
  _.filter = function(array, test) {
    ...
    _.each(array, function(value, index, array) {
      if (test(value, index, array) === true) result.push(value);
    });
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    return _.map(collection, function(value, index, collection){
      return (isFunction ? functionOrKey : value[functionOrKey]).apply(value, args);
    });
  };
  ...
  _.reduce = function(collection, iterator, accumulator) {
    if (arguments.length <= 2) accumulator = _.first(collection);
    _.each(collection, function(value, index, collection){
  ...
  _.shuffle = function(array) {
    ...
    var result = [];
    while (copy.length > 0) {
      result.push(copy.splice(Math.floor(Math.random() * copy.length),1).shift());
    }
    return result;
  };
  ...
  _.zip = function() {
    var result = [];
    var lengths = _.map(arguments, function(item){return item.length });
    var max = Math.max.apply(this, lengths);
    for (var i = 0; i < max; i++) {
      result[i] = [];
      for (var k = 0; k < arguments.length; k++) {
        result[i].push(arguments[k][i]);
      };
    };
    return result;
  };
